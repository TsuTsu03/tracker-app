...
export function PipelineClient({ leads: initial }: { leads: Lead[] }) {
  const [leads, setLeads] = useState<Lead[]>(initial);
  const [dragId, setDragId] = useState<string | null>(null);
  const [overStage, setOverStage] = useState<PipelineStage | null>(null);

  const move = (id: string, stage: PipelineStage) => {
    const prevStage = leads.find((l) => l.id === id)?.stage;
    if (prevStage === stage) return;
    // Optimistic update, then persist.
    setLeads((prev) => prev.map((l) => (l.id === id ? { ...l, stage } : l)));
    updateLeadStage(id, stage).then((res) => {
      if (res?.error && prevStage) {
