    ...
    <div className="animate-in">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_420px]">
        <div className="space-y-6">
          ...
          <Reengagement leads={leads} />
        </div>
        <Generator leads={leads} />
      </div>
    </div>
  );
}

function Reminders({ tasks: TASKS }: { tasks: Task[] }) {
  const cadence = [
...
function Reengagement({ leads: LEADS }: { leads: Lead[] }) {
...
function Generator({ leads: LEADS }: { leads: Lead[] }) {
  const [lead, setLead] = useState(LEADS[1] ?? LEADS[0]);
  ...
  const run = async () => {
    setLoading(true);
    setMsg(null);
    setMsg(await aiFollowUpMessage({
      name: lead.fullName,
    ...
    return (
      <Card className="lg:sticky lg:top-20 lg:self-start">
        <p className="py-8 text-center text-sm text-slate-500">
          Add a lead first to generate follow-up messages.
        </p>
      </Card>
    );
  }
